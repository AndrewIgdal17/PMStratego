...
5. **`node --test test/` (bare directory argument) fails outright on the Node version available during planning** (v23.6.1) — it errors with `Cannot find module '.../test'` instead of scanning the directory. `node --test` with no path argument does correctly auto-discover nested `test/**/*.test.js` files by default and was verified to work. Fixed Task 1's `package.json` test script from `"node --test test/"` to `"node --test"`. All per-task `Run:` commands elsewhere in this plan already pass explicit `.test.js` file paths (verified working) rather than a bare directory, so no other steps needed this fix.
