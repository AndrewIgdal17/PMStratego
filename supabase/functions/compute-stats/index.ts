...
Deno.serve(async (req) => {
  ...
  const marshalFights = moves.filter(
    (m: Move) => m.attacker_rank === "10" && m.defender_rank === "10",
  );
  ...
  for (const slot of [1, 2] as const) {
    ...
    const spyCombats = moves.filter((m: Move) => {
      if (m.player_slot === slot && m.attacker_rank === "SPY") return true;
      if (m.player_slot !== slot && m.defender_rank === "SPY") {
    ...
    const spyKills = moves.filter(
      (m: Move) =>
        m.player_slot === slot && m.attacker_rank === "SPY" && m.outcome === "ATTACKER_WINS",
    ).length;

    const myBombs = playerPieces.filter((p: Piece) => p.rank === "BOMB");
    const bombsDetonated = moves.filter(
      (m: Move) =>
        m.player_slot !== slot &&
        m.move_type === "attack" &&
        m.defender_rank === "BOMB" &&
        m.outcome === "DEFENDER_WINS",
    ).length;

    const myMiners = playerPieces.filter((p: Piece) => p.rank === "3");
    ...
    let winByNomoves = 0;
    if (won && lastMove) {
      if (lastMove.defender_rank === "FLAG") winByFlag = 1;
      else winByNomoves = 1;
    }
    ...
    const bombDefuses = playerMoves.filter(
      (m: Move) =>
        m.attacker_rank === "3" && m.defender_rank === "BOMB" && m.outcome === "ATTACKER_WINS",
    ).length;
    const enemyScoutsDead = enemyPieces.filter((p: Piece) => p.rank === "2" && !p.alive).length;
    const highPiecesLost = playerPieces.filter(
      (p: Piece) => !p.alive && ["8", "9", "10"].includes(p.rank),
    ).length;

    const newAchievements: string[] = [];
    if (spyKills > 0) newAchievements.push("kingmaker");
    if (bombDefuses >= 3) newAchievements.push("bomb_squad");
    if (won && lastMove?.attacker_rank === "3" && lastMove?.defender_rank === "FLAG") {
